...
qlonglong ModuloClientes::ultimoRegistroDeClienteEnBase(QString _tipoCliente)const {
    ...
    if(conexion){
        ...
        if(query.exec("select codigoCliente from Clientes where tipoCliente='"+_tipoCliente+"' order by cast(codigoCliente as unsigned) desc limit 1")) {
...
QString ModuloClientes::retornaDescripcionDeCliente(QString _codigoCliente,QString _tipoCliente) const {
    ...
    if(conexion){
        ...
        if(query.exec("select concat(C.razonSocial,' - ',C.nombreCliente,' - ',(case when C.rut is null then '' else C.rut end)) from Clientes C  where C.codigoCliente='"+_codigoCliente+"' and C.tipoCliente='"+_tipoCliente+"'")) {
